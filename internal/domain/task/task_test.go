...
import (
	...
	"gitlab.com/sukharnikov.aa/mail-service-task/internal/mocks/mock_analytics"
...
func (s *unitTestSuite) TestCreateTaskOk() {
	...
	man := new(mock_analytics.MockAnalytics)
	logger, _ := zap.NewProduction()
	taskIdUuid, err := uuid.FromString(taskId)
	s.NoError(err, "bad task id provided for test")
	srv := task.New(mts, mm, ma, man, logger.Sugar(), func() uuid.UUID {
		return taskIdUuid
	})
